...
2. GitHub Actions가 `config/cadence.json`에 정의된 요일마다 자동 실행되어, 대기 중인 글 중 가장 오래된
   ...
   올립니다.
3. 사람이 실제 에디터에서 발행합니다. **티스토리는 이슈의 붙여넣기용 HTML을 그대로 붙여넣어도 되지만,
   네이버는 초안을 참고해서 직접 타이핑하는 것을 권장합니다** (네이버가 붙여넣기/타이핑 패턴을 감지해
   저품질로 판단한다는 설이 공식 확인된 바는 없지만, 확인 안 된 리스크도 보수적으로 피하는 이 프로젝트의
   기존 방향과 일치하며 비용이 크지 않은 예방 조치이기 때문).
